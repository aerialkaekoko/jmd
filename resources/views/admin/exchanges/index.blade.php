                ...
                <tbody>
                    @foreach($exchange as $key => $exchange)
                        <tr data-entry-id="{{ $exchange->id }}">
                            ...
                            <td>
                              {{ date('Y-m-d',strtotime($exchange->updated_at))}}
                            </td>
                            <td>
                                {!! substr($exchange->exchange_usd / $exchange->exchange_thb,0,7) ?? '' !!}
                            </td>
                            <td>
                                {!! substr($exchange->exchange_usd * $exchange->exchange_thb,0,7) ?? '' !!}
                            </td>
                            <td>
                                {!! substr(($exchange->exchange_usd / $exchange->exchange_mmk) * $exchange->exchange_thb,0,7) ?? '' !!}
                            </td>                            
                            <td>
                                {!! substr($exchange->exchange_usd/ $exchange->exchange_mmk,0,7) ?? '' !!}
                            </td>
                            <td>
                                {!! substr($exchange->exchange_usd / $exchange->exchange_lak,0,7) ?? '' !!}
                            </td>
                            <td>
                                {!! substr($exchange->exchange_lak * $exchange->exchange_usd,0,7) ?? '' !!}
                            </td>
                            <td>
                                {{--
                                @can('exchanges_show')
